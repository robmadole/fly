...
function getSourceMap (sourceFilename) {
  if (parsedSourceMaps[sourceFilename]) { return parsedSourceMaps[sourceFilename] }

  const sm = global.sourceMaps[sourceFilename]
  if (!sm) { return }

  if (typeof SourceMapConsumer === 'undefined') { SourceMapConsumer = require('source-map').SourceMapConsumer }
