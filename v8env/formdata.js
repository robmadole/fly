...
export default function formDataInit (ivm, dispatcher) {
  function FormData (form) {
    ...
    for (var i = 0; i < form.elements.length; ++i) {
      ...
      if (element.name !== '') { this.append(element.name, element.value) }
    }
  }
  ...
  FormData.prototype = {
    ...
    get: function get (name) {
      ...
      for (var index = 0; index < this._data.length; ++index) {
        if (this._data[index][0] === name) { return this._data[index][1] }
      }
      return null
    },
    ...
    getAll: function getAll (name) {
      var sequence = []
      for (var index = 0; index < this._data.length; ++index) {
        if (this._data[index][0] === name) { sequence.push(this._data[index][1]) }
      }
      return sequence
    },
      ...
      for (var index = 0; index < this._data.length; ++index) {
        if (this._data[index][0] === name) {
          ...
          while (index < this._data.length) {
            if (this._data[index][0] === name) { this._data.splice(index, 1) } else { ++index }
          }
          return
        }
      }
