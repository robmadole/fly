...
export function fireEvent (ivm, name, ...args) {
  ...
  try {
    ...
    if (cb instanceof ivm.Reference) { cb.apply(undefined, [err.toString()]) }
  }
}
...
function fireFetchEvent (ivm, url, req, body, callback) {
  ...
  if (typeof fn !== 'function') { return selfCleaningCallback.apply(null, ["No 'fetch' event listener attached."]) }

  if (fn(fetchEvent) instanceof Promise) { return selfCleaningCallback.apply(null, ["'fetch' event handler function cannot return a promise."]) }

  if (!fetchEvent.respondWithEntered) { return selfCleaningCallback.apply(null, ['respondWith was not called for FetchEvent']) }
}
...
function fireFetchEndEvent (ivm, url, nodeReq, nodeRes, err, done) {
  ...
  if (listeners.length === 0) { return selfCleaningCallback.apply() }
